...
CIFAR-100 與 CIFAR-10 使用相同的圖片來源（32×32 彩色圖），但分類數從 10 擴充至 100，難度顯著提升。本模組在 CIFAR-10 ResNet 的基礎上加深一層（256→512），並採用 GPU 資料增強、Mixed Precision（AMP）與 CosineAnnealingLR，目標在 100 個 epoch 內達到 65%+ 測試準確率。
...
StepLR 以固定步距階梯式降低 lr，在多分類任務中容易在降 lr 前後出現明顯的準確率震盪。CosineAnnealingLR 以餘弦曲線平滑地從 0.001 衰減至接近 0，整體訓練曲線更穩定，尤其在 epoch 50 ~ 100 的收斂階段效果更佳。
...
| Dropout | 0.5（FC） | 0.3（FC） | 0.3（FC） |
| LR Scheduler | 無 | CosineAnnealingLR | CosineAnnealingLR |
| 資料增強 | 無 | Crop + HFlip + ColorJitter（GPU） | Crop + HFlip + ColorJitter（GPU） |
| Mixed Precision | AMP | AMP | AMP |
| Batch Size | 128 | 256 | 128 |
| Epochs | 50 | 100 | 100 |
| 預期 Test Acc | ~99.5% | ~88% | ~65% |
