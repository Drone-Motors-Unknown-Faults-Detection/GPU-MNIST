...
透過常見公開資料集（MNIST、CIFAR-10）在不同運算裝置上訓練 CNN／類 ResNet 模型，比較 **CPU、NVIDIA CUDA、Apple Silicon MPS** 等環境下的訓練行為；訓練過程可選擇將紀錄匯出為文字檔。
...
| `src/cifar10.py` | CIFAR-10：簡化 ResNet 訓練腳本 |
...
| `logs/MNIST/`、`logs/CIFAR10/` | 訓練輸出紀錄（執行時產生，勿隨意納入版本庫） |
| `README.md` | 使用者說明（含 MPS 章節） |
| `CNN_GUIDE.md` | 額外指南（若存在） |
| `mnist.sh` / `cifat10.sh` | 啟用 `venv` 後執行對應腳本（專案根目錄需有 `venv/`） |
...
1. **裝置相關**：新增或調整「選 GPU／MPS／CPU」時，請優先擴充 **`src/device.py`**，再在 `mnist.py` / `cifar10.py` 呼叫，避免各檔各寫一套。
...
4. **版本庫**：不要主動 commit `./data/`、下載的權重、以及 **`logs/`** 內訓練產生的 txt（除非使用者明確要求）。
